...
public class PairRDDrightOuterJoin implements Serializable {
  ...
  public void using() {
    ...
    // rightOuterJoin类似于SQL中的有外关联right outer
    // join，返回结果以参数中的RDD为主，关联不上的记录为空。只能用于两个RDD之间的关联，如果要多个RDD关联，多关联几次即可。
    // 参数numPartitions用于指定结果的分区数
    // 参数partitioner用于指定分区函数
    JavaPairRDD<String, Tuple2<Optional<Integer>, Integer>> rightOuterJoin = javaPairRDD
        .rightOuterJoin(javaPairRDD2);
    System.err.println(rightOuterJoin.collect());
    // [(4,(Optional[-4],12)), (4,(Optional[-4],12)), (5,(Optional[-5],15)),
    // (3,(Optional[-3],9)), (3,(Optional[-3],9))]
  }

  public static void main(String[] args) {
    new PairRDDrightOuterJoin().using();
    sc.close();
  }
